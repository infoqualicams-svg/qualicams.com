...
import { products as mockProducts, testimonials } from '@/lib/mock-data';
import { getProducts } from '@/lib/firebase-admin';
import { validateProducts, type SafeProduct } from '@/lib/validation';
    ...
    const loadFeaturedProducts = async () => {
      try {
        ...
        if (products.length === 0) {
          ...
          const safeProducts = validateProducts(mockProducts as any[]);
          setFeaturedProducts(safeProducts.slice(0, 4));
        } else {
          const safeProducts = validateProducts(products);
          setFeaturedProducts(safeProducts.slice(0, 4));
        }
      } catch (error) {
        console.warn('Failed to load featured products, using mock data:', error);
        const safeProducts = validateProducts(mockProducts as any[]);
        setFeaturedProducts(safeProducts.slice(0, 4));
      } finally {
        setLoading(false);
